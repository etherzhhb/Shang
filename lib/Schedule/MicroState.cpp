...
ucOpExpressionTrait::getHashValue(ucOp Op) {
  unsigned Hash = 0;
  ucOperand ThisOpCode = Op.OpCode;

  ThisOpCode.clearPredSlot();

  Hash = (unsigned)getMachineOperandHashValue(ThisOpCode) + Hash * 37;

  if (Op.isControl())
    Hash = (unsigned)getMachineOperandHashValue(Op.getPredicate()) + Hash * 37;

  for (unsigned i = 0, e = Op.getNumOperands(); i != e; ++i)
    Hash = (unsigned)getMachineOperandHashValue(Op.getOperand(i)) + Hash * 37;
